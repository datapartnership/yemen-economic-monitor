...
Finally, the team analyzed the impact of flaring on overall findings. Using the flare datasets described in the previous section ("Data"), the team recalculated monthly nightime light sums using the annual flaring data as a "mask". With this mask, we can generate estimated sums of nighttime lights (by governorate and by control area) for areas known to have flaring events and those without.

The line charts, below, show sum of lights by control area. It is noted that flaring activity is only observed in IRG-controlled areas. 
